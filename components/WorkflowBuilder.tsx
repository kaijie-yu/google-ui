...
import { Play, Save, Plus, Trash2, GripVertical, ChevronRight, Lock } from 'lucide-react';
...
export const WorkflowBuilder: React.FC<WorkflowBuilderProps> = ({ 
  ...
  const handleSelectWorkflow = (id: string) => {
    ...
    if (wf) {
      ...
      setEditingWorkflow({ ...wf }); // Create a copy to edit
      setExecutionLog([]);
    }
  };
  ...
  const runTest = async () => {
    ...
    setIsRunning(true);
    setExecutionLog(['Initializing test environment...', 'Browser started.']);
    ...
    setExecutionLog(prev => [...prev, 'Test Execution Finished: PASSED']);
    setIsRunning(false);
  };
    ...
    <div className="flex h-[calc(100vh-140px)] gap-6">
      ...
      <div className={`w-64 flex-shrink-0 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-opacity duration-300 ${isRunning ? 'opacity-60 pointer-events-none' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto relative">
           {isRunning && (
            <div className="absolute inset-0 z-10 bg-slate-50/50 flex items-center justify-center">
            </div>
           )}
          {workflows.map(wf => (
            <div 
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                 <button 
                  onClick={handleSave} 
                  disabled={isRunning}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors font-medium ${isRunning ? 'text-slate-400 bg-slate-50' : 'text-slate-600 bg-slate-100 hover:bg-slate-200'}`}
                >
                  <Save size={18} /> Save
                </button>
                <button 
                ...
                <div className="flex-1 p-4 overflow-y-auto font-mono text-xs space-y-1">
                  {executionLog.length === 0 ? (
                    ...
                    executionLog.map((log, i) => (
                      <div key={i} className={`${log.includes('Success') ? 'text-green-400' : log.includes('PASSED') ? 'text-green-300 font-bold' : 'text-slate-300'}`}>
                        {log}
                      </div>
                    ))
                  )}
                  {isRunning && <div className="text-amber-400 animate-pulse">Running Test...</div>}
                </div>
              </div>
            </div>
          </>
